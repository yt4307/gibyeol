...
기준일: 2026-08-28. `main`의 아래 commit까지 로컬 구현과 검증을 마쳤다. 외부 계정·비밀키가 필요한 배포 작업은 완료로 오인하지 않도록 별도 표시한다.
...
| M6 mailbox/tlock | 완료 | `da83851`, GPK1/X25519/tlock 경계와 Quicknet 실 round 왕복 |
| M7 contract | 완료 | `e3533aa`, `c14d312`, 7 tests와 1,000 fuzz runs |
...
| M10 email/recovery | 로컬 완료 | `582ab05`, `c63e3fe`, OTP/HMAC/recovery/webhook 26개 backend test 일부 |
| M11 발송 UI | 로컬 완료 | `d56555b`, static build와 Storybook |
| M12 받은 편지 UI | 로컬 완료 | `abd6002`, 직접 logs/calldata 조회와 검증·복호화·복구 flow |
...
- backend: 30 tests, 83 assertions 성공
- contract: 7 tests 성공, fuzz 1,000 runs 성공
- Quicknet 실제 round `31680670`에서 tlock encrypt/decrypt 왕복 성공
...
6. 실제 Chrome/Safari/Firefox WebAuthn PRF 및 cross-site cookie smoke를 수행할 브라우저 환경
