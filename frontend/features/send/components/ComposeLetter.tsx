...
import { formatMediaBytes, type MediaPreprocessingSummary } from "../data/media-preprocessing";
...
export function ComposeLetter({ draft, busy, error, mediaSummary, onChange, onSubmit, onReset }: ComposeLetterProps) {
  ...
  return <Form onSubmit={submit}>
    ...
    <Field><span>사진·타임랩스 (선택)</span><input key={draft.letterId} name="media" type="file" accept="image/webp,image/jpeg,image/png,video/webm,video/mp4" multiple disabled={draft.stage !== "DRAFT"} onChange={(event) => { const files = Array.from(event.currentTarget.files ?? []); setSelection(files.length ? { letterId: draft.letterId, count: files.length, bytes: files.reduce((total, file) => total + file.size, 0) } : null); }} /><MediaPolicy>사진은 최대 2,048px로 줄여 WebP 우선 변환하고, WebM·MP4 영상은 재생 가능 여부를 확인합니다. 암호화된 최종 소포는 10 MiB 이하여야 합니다.</MediaPolicy>{currentSelection && <MediaStatus>선택 {currentSelection.count}개 · 원본 {formatMediaBytes(currentSelection.bytes)}</MediaStatus>}{mediaSummary && <MediaStatus>전처리 완료 · {formatMediaBytes(mediaSummary.originalBytes)} → {formatMediaBytes(mediaSummary.processedBytes)} · 예상 소포 {formatMediaBytes(mediaSummary.estimatedArchiveBytes)}</MediaStatus>}</Field>
