...
export function useEmailRegistration(address?: string) {
  ...
  const requestCode = useCallback(async (email: string) => {
    ...
    } catch (cause) { setError(userFacingErrorMessage(cause, "이메일 인증 요청에 실패했습니다.")); throw cause; }
  ...
  const verifyCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (!response.ok) throw await apiResponseError(response, "인증번호를 확인해 주세요.");
      setVerifiedAddress(address ?? null);
    } catch (cause) { setError(userFacingErrorMessage(cause, "이메일 인증에 실패했습니다.")); throw cause; }
    finally { setBusy(false); }
  }, [address]);
