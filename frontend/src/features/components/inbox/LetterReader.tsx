...
export function LetterReader({ letter }: { letter: OpenedLetter }) {
  return <Paper id="opened-letter" tabIndex={-1} aria-label="봉인을 푼 기별"><Label>봉인을 푼 기별</Label><Message>{letter.message}</Message>{letter.media.length > 0 && <Media>{letter.media.map((item) => item.kind === "video" ? <video key={item.url} src={item.url} controls playsInline aria-label="기별에 첨부된 타임랩스" /> : <img key={item.url} src={item.url} alt="기별에 첨부된 사진" />)}</Media>}</Paper>;
}
