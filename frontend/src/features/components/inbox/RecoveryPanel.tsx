...
export function RecoveryPanel({ busy, onRequestCode, onRecover, onCancel }: RecoveryPanelProps) {
  ...
  return <Panel><Label>패스키 분실 복구</Label><h3>이메일로 본인 확인</h3><p>최근 이메일 인증 뒤 블록체인에 등록된 키와 일치하는 복구 키만 이 브라우저의 임시 키로 다시 봉인합니다.</p><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="email@example.com" /><button disabled={busy || !email} onClick={() => { void onRequestCode(email).then(() => setSent(true)).catch(() => undefined); }}>{sent ? "인증번호 다시 보내기" : "인증번호 받기"}</button>{sent && <><input inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value)} placeholder="6자리 인증번호" /><button disabled={busy || code.length !== 6} onClick={() => onRecover(code)}>메일박스 복구</button></>}<button className="cancel" onClick={onCancel}>취소</button></Panel>;
}
