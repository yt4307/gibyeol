...
const CancelButton = styled.button`min-height:44px;padding:0 var(--space-3);border:0;color:var(--color-text-muted);background:transparent;cursor:pointer;&:disabled{opacity:.55;cursor:default;}`;
...
const ErrorText = styled.p`grid-column: 1 / -1; color: var(--color-status-error); font-size: var(--font-size-100);&::before{margin-right:var(--space-2);content:"⚠";}`;
const RetryButton = styled.button`flex:0 0 auto;min-height:44px;padding:0 var(--space-3);border:1px solid var(--color-status-error);border-radius:3px;color:var(--color-text-primary);background:transparent;cursor:pointer;&:disabled{opacity:.55;cursor:default;}`;
