...
export function EmailRegistration({ verified, codeSent, busy, error, onRequestCode, onVerifyCode }: EmailRegistrationProps) {
  ...
  return <Panel><div><Label>도착 안내 이메일</Label><h3>{verified ? "도착 안내 이메일 인증 완료" : "기별 도착 안내받기"}</h3><p>이메일은 서버에서 암호화하고, 인증번호 원문은 저장하지 않습니다.</p></div>{!verified && <Fields><input type="email" value={email} disabled={busy || codeSent} onChange={(event) => setEmail(event.target.value)} placeholder="email@example.com" />{!codeSent ? <button disabled={busy || !email} onClick={() => onRequestCode(email)}>인증번호 받기</button> : <><input inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} placeholder="6자리 인증번호" /><button disabled={busy || code.length !== 6} onClick={() => onVerifyCode(code)}>이메일 확인</button></>}</Fields>}{error && <ErrorText role="alert">{error}</ErrorText>}</Panel>;
}
