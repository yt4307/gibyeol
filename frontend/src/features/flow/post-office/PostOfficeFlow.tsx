...
export function PostOfficeFlow({ view = "send" }: PostOfficeFlowProps) {
  ...
  return <Shell>
    ...
    <Workspace>
      <WalletPanel address={wallet.session?.address} busy={wallet.busy} restoring={wallet.restoring} error={wallet.error} onConnect={() => { void wallet.connect(); }} onChangeWallet={() => { void wallet.changeWallet(); }} />
